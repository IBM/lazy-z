import { paramTest, getType, typeCheck, arrTypeCheck } from "./values.js";
import { contains, eachKey, prettyJSON } from "./shortcuts.js";
 ...
 * @param {string} str name
 * @param {Array<string>} ignoreWords ignore words found in array
 * @return {string} empty string if not found, composed string otherwise
 */
function titleCase(str, ignoreWords) {
  let ignoreWordsLowerCase = [];
  if (ignoreWords) {
    arrTypeCheck("titleCase expects ignoreWords to be an array of", "string", ignoreWords);
    ignoreWords.forEach((word) =>
      ignoreWordsLowerCase.push(word.toLowerCase()),
    );
  }
  if (str) {
    ...
    while (splitStr.length > 0) {
      let item = splitStr.shift();
      if (ignoreWords && contains(ignoreWordsLowerCase, item.toLowerCase())) {
        composedStr +=
          item.toLowerCase() +
          (splitStr.length === 0 ||
          item.match(/^\d$/g) !== null ||
          item.match(/^[A-Z]$/g) !== null
            ? ""
            : " ");
      } else {
        composedStr +=
          (item === "And" && composedStr.length > 0 ? "and" : item) +
          // add space if not last item and not only number
          (splitStr.length === 0 ||
          item.match(/^\d$/g) !== null ||
          item.match(/^[A-Z]$/g) !== null
            ? ""
      ...
      // add space between ALL CAPS WORDS and other words
      if (splitStr.length > 0 && item.length === 1) {
        if (
          splitStr[0].length > 1 ||
          // if the current item is a captial letter and the next is a number
          (item.match(/^[A-Z]$/g) !== null &&
            splitStr[0].match(/^\d$/g) !== null) ||
          // or the current item is a number and the next is a letter
          (splitStr[0].match(/^[A-Z]$/g) !== null &&
            item.match(/^\d$/g) !== null)
        ) {
          composedStr += " ";
        }
