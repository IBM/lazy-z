...
import { typeCheck, paramTest, getType, zoneTest } from "./values.js";

/**
 * Callback function for object sort
 * @param {Object} a object a
 * @param {Object} b object b
 * @param {string=} key string key
 * @returns {number} 1, 0, -1
 * @throws will throw an error if params are not correct type
 */
function azObjectSort(a, b, key) {
  paramTest(
    "azObjectSort",
    "a",
    "object",
    ...
    .group((exp) => {
      exp
        .group((exp) => {
          exp
            .group((exp) => {
              exp.literal("2").group((exp) => {
                exp.literal("5").set("0-6").or().set("0-4").set("0-9");
              });
            })
            .or()
            .group((exp) => {
...
/**
 * Test if a value is array of objects
 * @param {*} value value
...
function isArrayOfObjects(value) {
  ...
  if (isArray(value)) {
    if (getType(value[0]) === "object") isArrayOfObjects = true;
  }
 ...
 * @param {*} value
 * @param {boolean=} lazy true if can be undefined defined
 * @returns {boolean} true if null or empty string
 */
function isNullOrEmptyString(value, lazy) {
  return (!value && lazy) || value === null || value === "";
}
