const { contains, containsKeys } = require("./shortcuts");
const { getObjectFromArray, transpose, carve, splat } = require("./objects");
const { getType, arrTypeCheck, typeCheck, containsAny } = require("./values");
const { keys } = require("regex-but-with-words/lib/utils");
/**
...
function revision(arrayOrObject, parent) {
  ...
  this.push = function (templateOrParams, params) {
    ...
    this.data.push(params ? newData : templateOrParams);
    return this;
  };
  ...
  this.set = function (key, templateOrParams, params) {
    ...
    let newData = params || (!params && isTemplate) ? {} : templateOrParams;
    if (params || isTemplate) {
      this.buildDefaultParams(newData, templateOrParams, params);
    }
    this.data[key] = params ? newData : templateOrParams;
    return this;
  };
