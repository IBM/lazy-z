...
import { initMockAxios, initRecursiveMockAxios } from "../lib/axios-mocks.js";
...
describe("axios mocks", () => {
  ...
  describe("recursive axios", () => {
    describe("main function", () => {
      it("should return a promise when called with params on resolve", () => {
        let { axios } = initRecursiveMockAxios([{ data: true }], false, true);
        return axios({ params: "yes" }).then((data) => {
          assert.isTrue(data.data.data, "it should be true");
        });
      });
      it("should reject when error is true", () => {
        let { axios } = initRecursiveMockAxios([{ data: true }], true, true);
        return axios({ params: "yes" }).catch((data) => {
          assert.isUndefined(data.data, "it should be true");
        });
      });
    });
    describe("get function", () => {
      it("should return a promise when called with params on resolve", () => {
        let { axios } = initRecursiveMockAxios({ data: true });
        return axios.get({ params: "yes" }).then((data) => {
          assert.isTrue(data.data.data, "it should be true");
        });
      });
